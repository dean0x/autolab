---
name: autoresearch-arena
description: >-
  This skill should be used when the user asks to "set up arena", "run competing
  agents", "pollinate results", "check leaderboard", or needs to coordinate
  multi-agent research competitions. Provides parallel strategy exploration via autoarena.
...
Run multi-agent research competitions with autoarena. Parallel agents with diverse strategies explore the search space faster than a single agent.
...
> than a single agent. Use arena to diversify approaches and cross-pollinate wins.
...
This creates:
- One git branch per agent (`arena/mar15-agent-1`, `arena/mar15-agent-2`, etc.)
- Each branch gets a `program.md` with a different strategy
- An `arena.json` config file tracking the competition
...
```bash
git checkout arena/mar15-agent-1
...
```bash
autoarena status          # quick overview with leader
...
This reads the leader's results, finds their most impactful experiments, and writes `arena-hints.md` to the repo root. All agents can read this file regardless of their branch -- it contains winning experiments, code diffs, and incorporation suggestions.
...
autoarena export --format tsv -o arena-results.tsv
```
...
```
autoarena --help
autoarena init --agents N --tag TAG [--base-branch BRANCH]
autoarena status [--quiet]
autoarena leaderboard [--detailed]
autoarena pollinate
autoarena export --format json|tsv [-o FILE]
```
...
Typical flow: **init arena** -> agents independently **run** -> **evaluate** -> **steer** -> **monitor leaderboard** -> **pollinate** -> repeat.
...
| Launching arena with < 3 agents | 3+ agents needed for meaningful strategy diversity |
