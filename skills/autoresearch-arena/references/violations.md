# Autoresearch Arena — Violation Examples

Expanded anti-pattern examples showing what goes wrong when arena is misused.
...
- You lose the primary advantage of arena: diverse parallel exploration.
...
```bash
autoarena init --agents 3 --tag experiment-run
...
Running the arena for 20+ experiments per agent without ever running `autoarena pollinate`.
...
- The collective knowledge of the arena is fragmented.
...
Running `autoarena pollinate` after only 2-3 experiments per agent.
...
Launching an arena with only 1-2 agents.

### What Goes Wrong

- 1 agent: no competition, no diversity. Just use autosteer directly.
- 2 agents: binary comparison, minimal strategy diversity. The "arena" adds overhead without meaningful benefit.
...
Launching the arena and not checking `status` or `leaderboard` until the end.
...
- You can't make informed decisions about custom strategy allocation for the next arena run.
- The `--detailed` flag exists for this purpose — not using it is leaving data on the table.

### Correct Approach

Use `autoarena leaderboard --detailed` to see strategy-level metrics. Use this data to inform:
- Which strategies to duplicate if adding more agents
- Whether to run a follow-up arena with adjusted allocations
- What direction to pursue in single-agent mode after the arena ends
